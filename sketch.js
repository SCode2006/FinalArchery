  ...
  for (var i = 0; i < playerArrows.length; i++) {
    showArrows(i, playerArrows);
      for (var j = 0; j < computerArrows.length; j++) {
        if (playerArrows[i] !== undefined && computerArrows[j] !== undefined) {
          var collision = Matter.SAT.collides(playerArrows[i].body, computerArrows[j].body);
          if (collision.collided) {
            computerArrows[j].remove(j);
